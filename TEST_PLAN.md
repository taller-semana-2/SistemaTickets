...
SonarQube se utilizará como herramienta de análisis estático y medición de cobertura para **todos los servicios**, tanto para los tests ya existentes como para los nuevos que se implementen.

### Qué analiza SonarQube en este proyecto

| Dimensión | Qué mide | Relevancia para este proyecto |
|---|---|---|
| **Cobertura de código** | % de líneas cubiertas por tests | Detecta qué capas (dominio, app, infra) no tienen tests |
| **Duplicación** | Bloques de código repetidos | Aplica dentro de cada servicio, no entre servicios |
| **Code smells** | Complejidad ciclomática, funciones largas | Especialmente crítico en handlers de RabbitMQ |
| **Bugs potenciales** | Análisis estático de errores lógicos | Validaciones de eventos y transiciones de estado |
| **Security Hotspots** | Prácticas inseguras | Manejo de credenciales, variables de entorno |
| **Deuda técnica** | Tiempo estimado para resolver issues | Conecta con `DEUDA_TECNICA.md` ya documentada |

---

### Configuración de cobertura por servicio

Para generar el reporte que SonarQube consume, cada servicio debe ejecutar pytest con cobertura:
...
El archivo `coverage.xml` generado es el que SonarQube lee para calcular cobertura.

---

### Quality Gates — Umbrales definidos

Se propone aplicar un Quality Gate con los siguientes criterios. Un análisis **falla** si no se cumplen:

| Métrica | Umbral mínimo | Justificación |
|---|---|---|
| Cobertura en código nuevo | ≥ 80% | Todo código nuevo introducido debe tener tests |
| Cobertura total por servicio | ≥ 60% | Umbral realista dado el estado actual del proyecto |
| Duplicación | ≤ 3% | Dentro de un mismo servicio (no entre servicios) |
| Bugs críticos nuevos | 0 | Ningún bug bloqueante puede entrar al código base |
| Security Hotspots sin revisar | 0 | Todos los hotspots deben ser revisados y marcados |

---

### Cobertura esperada por capa (ticket-service como referencia)

| Capa | Tests que la cubren | Cobertura esperada |
|---|---|---|
| `domain/` | U1–U15 (unitarios puros) | 🔴 Debe ser ≥ 90% — es la capa más crítica |
| `application/` | U5–U12, I4–I5 | 🔴 Debe ser ≥ 80% |
| `infrastructure/` | I1–I3 | 🟡 Apuntar a ≥ 70% |
| `views.py` | U14–U15 | 🟡 Apuntar a ≥ 60% |

---

### Impacto de SonarQube sobre los tests ⚠️ y 🆕

| Test | Estado | Impacto en SonarQube |
|---|---|---|
| U33: `CreateUserUseCase` sin tests | ⚠️ archivo vacío | 🔴 `users-service` tendrá cobertura ~0% en capa de aplicación — Quality Gate falla |
| I11: `users-service` sin integración | ⚠️ archivo vacío | 🔴 API de usuarios sin cobertura alguna |
| I12: Consumer con schema inválido | 🆕 no existe | 🟡 Handlers de messaging tendrán cobertura baja |
| I13: Contrato de eventos | 🆕 no existe | 🟡 Serialización de eventos sin cobertura |
| I9/I10: Tests frágiles con Docker | ⚠️ requieren infra real | 🟡 SonarQube no los ejecutará en CI local — la cobertura de esa rama queda en cero |

> **Nota:** Los tests que requieren RabbitMQ real (I9, I10, S1, S2) deben marcarse con `@pytest.mark.integration` y excluirse del análisis local de SonarQube mediante el flag `--ignore` o un perfil de pytest separado. De lo contrario, fallarán el pipeline de CI al no tener broker disponible.

---

### Archivos a excluir del análisis de SonarQube

Algunos archivos no aportan valor al análisis y deben excluirse en `sonar-project.properties`:

```properties
# sonar-project.properties (por servicio)
sonar.exclusions=**/migrations/**, **/tests/**, **/__pycache__/**, **/manage.py, **/wsgi.py, **/asgi.py
