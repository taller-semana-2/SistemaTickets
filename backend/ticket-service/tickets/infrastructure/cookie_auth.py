...
This keeps consumer services stateless (no User DB lookups) while
supporting the HttpOnly cookie auth flow from users-service.
"""
...
class CookieJWTStatelessAuthentication(JWTStatelessUserAuthentication):
    ...
    Used in consumer services (ticket, notification, assignment) that
    do NOT have a local User table.
    """
